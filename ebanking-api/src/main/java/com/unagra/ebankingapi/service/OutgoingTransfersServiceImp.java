...
public class OutgoingTransfersServiceImp implements OutgoingTransfersService {
    ...
    public OutgoingTransfersResponse newTransferUNAGRA_OWN(OutgoingTransfersDTO outgoingTransfersDTO) {
        // 1.- if token sent doesn't valid, we can't continue...
        ...
        if (vlValidateToken == null) {
            throw new ResourceNotFoundException("¡Ups!, lo sentimos, el token proporcionado no tiene coincidencia con el proceso realizado por el clienteID [" + outgoingTransfersDTO.getCustomerid_origin().toString() + "], favor de proporcionar el token correcto para esta solicitud.");
        }
        ...
        // 3.- origin account sent must exist...
        Long vlAccountExists = accountsRepository.findAccountID(outgoingTransfersDTO.getAccountid_origin());
        if (vlAccountExists == null) {
            // notice...
            throw new ResourceNotFoundException("¡Ups!, lo sentimos, la cuentaID [" + outgoingTransfersDTO.getAccountid_origin() + "] no se encuentra registrada.");
        }

        // 4.- beneficiary accountID sent must be part of current accounts of customer logged in...
